// Gestor de Configurações - Manipula todas as configurações da aplicação
class SettingsManager {
    constructor() {
        this.settings = {
            ...
            refreshRate: '0',
            defaultLocation: '',
            // Limitares de temperatura (Celsius)
            tempThresholds: {
                veryCold: 0,
                cold: 10,
                mild: 20,
            ...
            // Limitares de velocidade do vento (km/h)
            windThresholds: {
                light: 5,
                moderate: 13,
                strong: 28,
    ...
    loadSettings() {
        ...
        if (savedSettings) {
            ...
            // Mesclar com padrões para garantir que todas as propriedades de limiar existem
            this.settings = { ...this.settings, ...parsedSettings };
            
            // Garantir que todos os objetos de limiar existem com valores padrão
    ...
    saveSettings() {
        ...
        // Guardar configurações de limiar
    ...
    applyTheme() {
        ...
        if (this.settings.theme === 'dark') {
            ...
            // Garantir que todos os elementos de texto têm cores brilhantes
    ...
    // Carregar configurações de limiar no formulário
    loadThresholdSettings() {
        const tempUnit = this.settings.temperatureUnit;
        
        // Limitares de temperatura
        ...
        // Limitares de velocidade do vento
    ...
    // Guardar configurações de limiar do formulário
    saveThresholdSettings() {
        const tempUnit = this.settings.temperatureUnit;
        
        // Limitares de temperatura - converter de volta para Celsius para armazenamento
        ...
        // Limitares de precipitação - converter de volta para mm para armazenamento
        const precipUnit = this.settings.precipitationUnit;
        ...
        // Limitares de velocidade do vento - converter de volta para km/h para armazenamento
        const windUnit = this.settings.windSpeedUnit;
    ...
    setupEventListeners() {
        // Only setup if we're on the settings page
        if (window.location.pathname.includes('settings.html')) {
            this.setupSettingsPageListeners();
        }
    }

    // Setup listeners specifically for settings page
